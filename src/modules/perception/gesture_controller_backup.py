...
class GestureController:
    ...
    def _count_fingers_robust(self, hand) -> tuple:
        """
        More robust finger counting method
        
        Returns:
            (finger_count, fingers_list) tuple
        """
        lmList = hand['lmList']
        ...
        # Thumb: Check if tip (4) is to the right/left of MCP (2) depending on hand
        thumb_tip = lmList[4]
        thumb_mcp = lmList[2]
        if hand['type'] == 'Right':
        ...
        # Other fingers: Check if tip is above MCP (lower y value = higher on screen)
        ...
        for tip_id, mcp_id in zip(finger_tips, finger_mcps):
            tip = lmList[tip_id]
            mcp = lmList[mcp_id]
            # If tip Y is less than MCP Y, finger is up (y increases downward)
            fingers.append(1 if tip[1] < mcp[1] - 10 else 0)  # -10 for threshold
    ...
    def detect_gestures(self, frame: np.ndarray) -> List[GestureData]:
        ...
        if hands:
            ...
            # Use robust finger counting instead of CVZone's fingersUp
            finger_count, fingers = self._count_fingers_robust(hand)
            
            # Debug: Print finger detection
            if self.logger:
                self.logger.info(f"Fingers detected: {finger_count} | Fingers up: {fingers} | Hand type: {hand.get('type', 'unknown')}")
            ...
            # Debug: Print gesture classification
            if self.logger and gesture_type:
                self.logger.info(f"Gesture: {gesture_type} | Value: {value} | Y-Position: {y_percent}%")
